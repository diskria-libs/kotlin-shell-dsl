...
open class Shell protected constructor(private var workingDirectory: File) {
    ...
    fun runAndGetOutput(command: String): String =
        startProcess(command).inputStream.bufferedReader().readText().trim()
    ...
    private fun startProcess(command: String): Process {
        ...
        return ProcessBuilder(arguments).directory(workingDirectory).redirectErrorStream(true).start()
    }

    private fun splitToArguments(command: String): List<String> =
        command
            .findAll(argumentRegex)
            .map { (argument) -> argument.trim(Constants.Char.DOUBLE_QUOTE, Constants.Char.SINGLE_QUOTE) }
            .toList()
    ...
    companion object {
        ...
        fun open(workingDirectoryPath: String = Constants.File.Path.CURRENT_DIRECTORY): Shell =
            Shell(workingDirectoryPath.toFile().asDirectory())
